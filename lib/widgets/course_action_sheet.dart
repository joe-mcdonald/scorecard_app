...
class CourseActionSheet extends StatefulWidget {
  ...
  final Function(List<int> par, List<int> mensHcap, List<int> womensHcap, List<String> tees, Map<String, List<int>> yardages, String selectedTee)
      onCourseDataLoaded;
  final List<int> par;
...
class _CourseActionSheetState extends State<CourseActionSheet> {
  Future<void> _loadCourseData(String course, String courseName) async {
    ...
    List<String> tees = csvData.map((row) => row[0].toString()).skip(5).toList();
    ...
    setState(() {
      widget.onCourseDataLoaded(par, mensHcap, womensHcap, tees, yardages, selectedTee);
      widget.onCourseSelected(courseName, selectedTee);
    });
  }

  CupertinoActionSheetAction _buildAction(String courseFile, String courseName, String courseLocation) {
    return CupertinoActionSheetAction(
      ...
      child: Column(
        children: [
          Text(courseName),
          Text(courseLocation, style: const TextStyle(fontSize: 12, color: CupertinoColors.systemGrey)),
        ],
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: CupertinoActionSheet(
        title: const Text('Courses', style: TextStyle(fontSize: 16, color: CupertinoColors.systemGrey)),
        message: const Text('Select a course.'),
        actions: [
          _buildAction('bigskygc', 'Big Sky Golf Club', 'Pemberton, BC'),
          _buildAction('highlandpacific', 'Highland Pacific Golf Course', 'Victoria, BC'),
          _buildAction('marinedrive', 'Marine Drive Golf Club', 'Vancouver, BC'),
          _buildAction('nicklausnorth', 'Nicklaus North Golf Club', 'Whistler, BC'),
          _buildAction('pheasantglen', 'Pheasant Glen Golf Resort', 'Nanaimo, BC'),
